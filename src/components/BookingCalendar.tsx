...
export function BookingCalendar({
  ...
  const renderCell = (day: number) => {
    ...
    let cellClass = "relative flex flex-col items-center justify-center rounded-xl w-full aspect-square text-sm font-semibold transition-all duration-200 cursor-pointer ";
    ...
    if (isPast) {
      ...
      cellClass += "bg-amber-500/10 text-amber-600 border border-amber-500/30 hover:bg-amber-500/20";
    } else {
      cellClass += "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 hover:bg-emerald-500/20 hover:shadow-sm";
    }
    ...
    return (
      ...
      >
        ...
        {count > 0 && !isPast && (
          <span className={`absolute -top-1 -right-1 flex items-center justify-center w-4.5 h-4.5 rounded-full text-[9px] font-bold ${
            isFull
        ...
        <div className="bg-card rounded-2xl border border-border p-6 shadow-sm animate-slide-down space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-display text-xl text-foreground">
              Bookings on {new Date(adminSelectedDay + "T00:00:00").toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
            </h4>
                ...
                >
                  <div className="flex-1 space-y-1.5">
                    ...
                    <div className="text-xs text-muted-foreground space-y-0.5">
                      <div>Pet: <strong className="text-foreground">{booking.petName}</strong> ({booking.breed}, {booking.age})</div>
                      <div>Medical: <strong className="text-foreground">{booking.medicalConditions || "N/A"}</strong></div>
                      {booking.selectedCommands.length > 0 && (
                        <div>Commands: <strong className="text-foreground">{booking.selectedCommands.join(", ")}</strong></div>
                      )}
